...
function extractFiles(filename) {
    ...
    const regex = /path:\s*['"]([^'"]+)['"][\s\S]*?content:\s*`([\s\S]*?)`/g;
    let match;
    const files = [];
...
const baseFiles = extractFiles(path.join(process.cwd(), 'src/androidCode/androidFiles.ts'));
const extraFiles = extractFiles(path.join(process.cwd(), 'src/androidCode/androidExtraFiles.ts'));
...
for (const file of allFiles) {
    ...
    // Remove the backslash escaping that might be in the TS file strings
    let cleanContent = file.content.replace(/\\`/g, '`').replace(/\\\$/g, '$');
...
console.log('Extraction complete! Extracted', allFiles.length, 'files.');
